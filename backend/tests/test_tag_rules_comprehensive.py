...
class TestTagRulesEdgeCases:
    ...
    async def test_update_rule_invalid_tag_format(self, client: AsyncClient, seed_categories):
        ...
        create_response = await client.post(
            "/api/v1/tag-rules", json={"name": "Update Tag Test", "tag": "bucket:shopping", "merchant_pattern": "test"}
        )
        rule_id = create_response.json()["id"]

        response = await client.patch(f"/api/v1/tag-rules/{rule_id}", json={"tag": "invalid-no-colon"})
        assert response.status_code == 400
    ...
    async def test_update_rule_nonexistent_tag(self, client: AsyncClient, seed_categories):
        ...
        create_response = await client.post(
            "/api/v1/tag-rules",
            json={"name": "Update Nonexistent Tag Test", "tag": "bucket:shopping", "merchant_pattern": "test"},
        )
        rule_id = create_response.json()["id"]

        response = await client.patch(f"/api/v1/tag-rules/{rule_id}", json={"tag": "bucket:does-not-exist-12345"})
        assert response.status_code == 400
...
class TestTagRuleMatching:
    ...
    async def test_account_source_matching(self, client: AsyncClient, seed_categories):
        ...
        await client.post(
            "/api/v1/tag-rules",
            json={"name": "Account Source Rule", "tag": "bucket:shopping", "account_source": "SPECIFIC-ACCOUNT"},
        )
    ...
    async def test_amount_min_only(self, client: AsyncClient, seed_categories):
        ...
        await client.post(
            "/api/v1/tag-rules", json={"name": "Min Amount Only", "tag": "bucket:shopping", "amount_min": 50.00}
        )
    ...
    async def test_amount_max_only(self, client: AsyncClient, seed_categories):
        ...
        await client.post(
            "/api/v1/tag-rules", json={"name": "Max Amount Only", "tag": "bucket:shopping", "amount_max": 50.00}
        )
    ...
    async def test_empty_merchant_pattern(self, client: AsyncClient, seed_categories):
        ...
        await client.post(
            "/api/v1/tag-rules",
            json={"name": "Empty Merchant Test", "tag": "bucket:shopping", "description_pattern": "purchase"},
        )
...
class TestApplyRules:
    ...
    async def test_apply_rules_skips_already_tagged(self, client: AsyncClient, seed_categories):
        ...
        await client.post(
            "/api/v1/tag-rules",
            json={"name": "Skip Tagged Test", "tag": "bucket:shopping", "merchant_pattern": "target"},
        )
    ...
    async def test_test_rule_with_matches(self, client: AsyncClient, seed_categories):
        ...
        create_response = await client.post(
            "/api/v1/tag-rules",
            json={"name": "Match Test", "tag": "bucket:shopping", "merchant_pattern": "testmerchant"},
        )
        rule_id = create_response.json()["id"]
    ...
    async def test_apply_returns_rule_stats(self, client: AsyncClient, seed_categories):
        ...
        await client.post(
            "/api/v1/tag-rules", json={"name": "Rule A", "tag": "bucket:shopping", "merchant_pattern": "rulea"}
        )

        await client.post(
            "/api/v1/tag-rules", json={"name": "Rule B", "tag": "bucket:groceries", "merchant_pattern": "ruleb"}
        )
...
class TestTagRuleTagApplication:
    ...
    async def test_bucket_tag_replaces_previous(self, client: AsyncClient, seed_categories):
        ...
        await client.post(
            "/api/v1/tag-rules",
            json={"name": "Initial Bucket", "tag": "bucket:groceries", "merchant_pattern": "replace store"},
        )
    ...
    async def test_apply_occasion_tag_via_rule(self, client: AsyncClient, seed_categories):
        ...
        await client.post(
            "/api/v1/tag-rules",
            json={"name": "Vacation Spending", "tag": "occasion:vacation-2025", "description_pattern": "vacation"},
        )
    ...
    async def test_priority_ordering_in_apply(self, client: AsyncClient, seed_categories):
        ...
        await client.post(
            "/api/v1/tag-rules",
            json={"name": "Low Priority", "tag": "bucket:other", "merchant_pattern": "priority", "priority": 10},
        )

        # Create high priority rule
        await client.post(
            "/api/v1/tag-rules",
            json={"name": "High Priority", "tag": "bucket:shopping", "merchant_pattern": "priority", "priority": 100},
        )
