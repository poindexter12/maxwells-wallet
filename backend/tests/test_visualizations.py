...
class TestSankeyFlow:
    ...
    async def test_sankey_with_income(self, client: AsyncClient, seed_categories):
        ...
        assert any(link["value"] == 5000.0 for link in data["links"])
    ...
    async def test_sankey_with_expenses(self, client: AsyncClient, seed_categories):
        ...
        assert any(link["value"] == 150.0 for link in data["links"])
    ...
    async def test_sankey_excludes_transfers(self, client: AsyncClient, seed_categories):
        ...
        assert not any(link["value"] == 500.0 for link in data["links"])
...
class TestTreemap:
    ...
    async def test_treemap_with_expenses(self, client: AsyncClient, seed_categories):
        ...
        await client.post(f"/api/v1/transactions/{txn1.json()['id']}/tags", json={"tag": "bucket:groceries"})
        await client.post(f"/api/v1/transactions/{txn2.json()['id']}/tags", json={"tag": "bucket:groceries"})
