...
class TestFiltersComprehensive:
    ...
    async def test_create_filter_minimal(self, client: AsyncClient):
        ...
        filter_data = {"name": "Minimal Test Filter"}
    ...
    async def test_create_filter_amount_range(self, client: AsyncClient):
        ...
        filter_data = {"name": "Amount Range Filter", "min_amount": -1000.0, "max_amount": -100.0}
        response = await client.post("/api/v1/filters", json=filter_data)
        assert response.status_code in [200, 201]
    ...
    async def test_create_filter_date_range(self, client: AsyncClient):
        ...
        filter_data = {"name": "Date Range Filter", "start_date": "2025-01-01", "end_date": "2025-12-31"}
        response = await client.post("/api/v1/filters", json=filter_data)
        assert response.status_code in [200, 201]
    ...
    async def test_create_filter_search_query(self, client: AsyncClient):
        ...
        filter_data = {"name": "Search Query Filter", "search": "AMAZON"}
        response = await client.post("/api/v1/filters", json=filter_data)
        assert response.status_code in [200, 201]
    ...
    async def test_create_filter_account(self, client: AsyncClient):
        ...
        filter_data = {"name": "Account Filter", "account_source": "AMEX-5678"}
        response = await client.post("/api/v1/filters", json=filter_data)
        assert response.status_code in [200, 201]
    ...
    async def test_create_filter_bucket(self, client: AsyncClient, seed_categories):
        ...
        filter_data = {"name": "Bucket Filter", "bucket": "groceries"}
        response = await client.post("/api/v1/filters", json=filter_data)
        assert response.status_code in [200, 201]
    ...
    async def test_create_filter_reconciliation(self, client: AsyncClient):
        ...
        filter_data = {"name": "Unreconciled Filter", "reconciliation_status": "unreconciled"}
        response = await client.post("/api/v1/filters", json=filter_data)
        assert response.status_code in [200, 201]
    ...
    async def test_get_filter_by_id(self, client: AsyncClient):
        ...
        create_response = await client.post("/api/v1/filters", json={"name": "Get Test Filter"})
    ...
    async def test_update_filter(self, client: AsyncClient):
        ...
        create_response = await client.post("/api/v1/filters", json={"name": "Update Test Filter"})
        filter_id = create_response.json().get("id")

        if filter_id:
            response = await client.patch(
                f"/api/v1/filters/{filter_id}", json={"name": "Updated Filter Name", "min_amount": -200.0}
            )
            assert response.status_code == 200
    ...
    async def test_update_nonexistent_filter(self, client: AsyncClient):
        ...
        response = await client.patch("/api/v1/filters/99999", json={"name": "Updated"})
        assert response.status_code == 404

    @pytest.mark.asyncio
    async def test_delete_filter(self, client: AsyncClient):
        """Delete a filter"""
        create_response = await client.post("/api/v1/filters", json={"name": "Delete Test Filter"})
        filter_id = create_response.json().get("id")
    ...
    async def test_apply_filter(self, client: AsyncClient, seed_transactions):
        ...
        create_response = await client.post(
            "/api/v1/filters", json={"name": "Apply Test Filter", "min_amount": -1000.0, "max_amount": 0}
        )
        filter_id = create_response.json().get("id")
    ...
    async def test_toggle_pin_filter(self, client: AsyncClient):
        ...
        create_response = await client.post("/api/v1/filters", json={"name": "Pin Test Filter"})
        filter_id = create_response.json().get("id")
    ...
    async def test_update_filter_with_accounts_list(self, client: AsyncClient):
        ...
        create_response = await client.post("/api/v1/filters", json={"name": "Accounts List Test"})
        filter_id = create_response.json().get("id")

        if filter_id:
            response = await client.patch(f"/api/v1/filters/{filter_id}", json={"accounts": ["account1", "account2"]})
            assert response.status_code == 200

    @pytest.mark.asyncio
    async def test_update_filter_with_reconciliation_status(self, client: AsyncClient):
        """Update filter with reconciliation status"""
        create_response = await client.post("/api/v1/filters", json={"name": "Reconciliation Test"})
        filter_id = create_response.json().get("id")

        if filter_id:
            response = await client.patch(f"/api/v1/filters/{filter_id}", json={"reconciliation_status": "matched"})
            assert response.status_code == 200
    ...
    async def test_create_and_apply_relative_date_filter(self, client: AsyncClient, seed_transactions):
        ...
        create_response = await client.post(
            "/api/v1/filters", json={"name": "Relative Date Filter", "date_range_type": "relative", "relative_days": 30}
        )
        filter_id = create_response.json().get("id")
    ...
    async def test_apply_filter_with_reconciliation_status(self, client: AsyncClient, seed_transactions):
        ...
        create_response = await client.post(
            "/api/v1/filters", json={"name": "Reconciliation Apply Test", "reconciliation_status": "unreconciled"}
        )
        filter_id = create_response.json().get("id")
