...
class TestQueryPatterns:
    ...
    async def test_transaction_list_constant_queries(self, perf_client, seed_large_dataset, query_counter):
        """
        ...
        assert queries_for_50 < queries_for_10 * 2, (
            f"Possible N+1: {queries_for_10} queries for 10 items, {queries_for_50} queries for 50 items"
        )

    async def test_dashboard_widgets_constant_queries(self, perf_client, seed_large_dataset, query_counter):
        """
        ...
        query_counter.reset()
        async with timed_request(query_counter):
            response = await perf_client.get(f"/api/v1/dashboards/{dashboard_id}/widgets")
        ...
        assert query_counter.count <= 5, (
            f"Widget loading is O(n): {widget_count} widgets, {query_counter.count} queries"
        )

    async def test_tags_list_no_usage_n_plus_one(self, perf_client, seed_large_dataset, query_counter):
        """
        ...
        assert query_counter.count <= 5, f"Tag listing may have N+1: {tag_count} tags, {query_counter.count} queries"

    async def test_budgets_with_spending_efficient(self, perf_client, seed_large_dataset, query_counter):
        """
        ...
        assert query_counter.count <= 20, (
            f"Budget spending calculation may be N+1: {len(budgets)} budgets, {query_counter.count} queries"
        )
...
class TestIndexUsage:
    ...
    async def test_date_range_uses_index(self, perf_client, seed_large_dataset, query_counter):
        """
        ...
        async with timed_request(query_counter) as timing:
            response = await perf_client.get(
                "/api/v1/transactions/", params={"start_date": "2024-06-01", "end_date": "2024-06-30", "limit": 100}
            )

        assert response.status_code == 200
        # Date-indexed query on 10k rows should be <200ms
        assert timing.duration_ms < 200, (
            f"Date range query slow ({timing.duration_ms:.0f}ms) - check index on transaction.date"
        )

    async def test_category_filter_uses_index(self, perf_client, seed_large_dataset, query_counter):
        """
        Category filtering should be efficient.
        """
        async with timed_request(query_counter) as timing:
            response = await perf_client.get("/api/v1/transactions/", params={"category": "groceries", "limit": 100})

        assert response.status_code == 200
        assert timing.duration_ms < 200, (
            f"Category filter slow ({timing.duration_ms:.0f}ms) - check index on transaction.category"
        )

    async def test_account_filter_uses_index(self, perf_client, seed_large_dataset, query_counter):
        """
        Account filtering should use index on account_tag_id.
        """
        async with timed_request(query_counter) as timing:
            response = await perf_client.get("/api/v1/transactions/", params={"account": ["chase"], "limit": 100})

        assert response.status_code == 200
        assert timing.duration_ms < 200, (
            f"Account filter slow ({timing.duration_ms:.0f}ms) - check index on transaction.account_tag_id"
        )
...
class TestAggregationEfficiency:
    ...
    async def test_bucket_aggregation_single_query(self, perf_client, seed_large_dataset, query_counter):
        """
        Bucket breakdown should use GROUP BY, not multiple queries.
        """
        query_counter.reset()
        async with timed_request(query_counter):
            response = await perf_client.get("/api/v1/reports/monthly-summary", params={"year": 2024, "month": 6})
    ...
    async def test_trend_aggregation_efficiency(self, perf_client, seed_large_dataset, query_counter):
        """
