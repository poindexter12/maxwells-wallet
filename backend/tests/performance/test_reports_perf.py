...
class TestReportPerformance:
    ...
    async def test_trends_report_weekly(
    ...
    async def test_trends_report_monthly(
        self, perf_client, seed_large_dataset, query_counter, thresholds
    ):
        ...
        timing.assert_under(
            thresholds.REPORT_GENERATION_MS,
            "Monthly trends too slow"
        )

    async def test_top_merchants_report(
        self, perf_client, seed_large_dataset, query_counter, thresholds
    ):
        ...
        timing.assert_under(
            thresholds.REPORT_GENERATION_MS,
            "Top merchants too slow"
        )
        query_counter.assert_max_queries(
            thresholds.MAX_QUERIES_REPORT,
            "Top merchants has too many queries"
        )

    async def test_sankey_flow_report(
        self, perf_client, seed_large_dataset, query_counter, thresholds
    ):
        ...
        timing.assert_under(
            thresholds.REPORT_GENERATION_MS,
            "Sankey flow too slow"
        )

    async def test_treemap_report(
        self, perf_client, seed_large_dataset, query_counter, thresholds
    ):
    ...
    async def test_month_over_month_report(
        self, perf_client, seed_large_dataset, query_counter, thresholds
    ):
        ...
        timing.assert_under(
            thresholds.REPORT_GENERATION_MS,
            "Month-over-month too slow"
        )

    async def test_spending_velocity_report(
        self, perf_client, seed_large_dataset, query_counter, thresholds
    ):
        ...
        timing.assert_under(
            thresholds.REPORT_GENERATION_MS,
            "Spending velocity too slow"
        )

    async def test_anomalies_report(
        self, perf_client, seed_large_dataset, query_counter, thresholds
    ):
        ...
        async with timed_request(query_counter) as timing:
            response = await perf_client.get(
                ...
                params={"year": 2024, "month": 6, "threshold": 2.0}
            )

        assert response.status_code == 200
        timing.assert_under(
            thresholds.REPORT_GENERATION_MS,
            "Anomaly detection too slow"
        )
...
class TestReportFilterPerformance:
    ...
    async def test_trends_with_bucket_filter(
        self, perf_client, seed_large_dataset, query_counter, thresholds
    ):
        ...
        timing.assert_under(
            thresholds.REPORT_GENERATION_MS,
            "Filtered trends too slow"
        )

    async def test_trends_with_account_filter(
        self, perf_client, seed_large_dataset, query_counter, thresholds
    ):
        ...
        timing.assert_under(
            thresholds.REPORT_GENERATION_MS,
            "Account-filtered trends too slow"
        )
