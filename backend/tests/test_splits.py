...
class TestTransactionSplits:
    ...
    async def test_set_single_split(self, client: AsyncClient, seed_categories):
        ...
        response = await client.put(
            f"/api/v1/transactions/{txn_id}/splits", json={"splits": [{"tag": "bucket:groceries", "amount": 50.00}]}
        )
        assert response.status_code == 200
        data = response.json()
    ...
    async def test_set_multiple_splits(self, client: AsyncClient, seed_categories):
        ...
        response = await client.put(
            ...
            json={
                "splits": [{"tag": "bucket:groceries", "amount": 60.00}, {"tag": "bucket:shopping", "amount": 30.00}]
            },
        )
        assert response.status_code == 200
        data = response.json()
    ...
    async def test_partial_split_allowed(self, client: AsyncClient, seed_categories):
        ...
        response = await client.put(
            f"/api/v1/transactions/{txn_id}/splits", json={"splits": [{"tag": "bucket:groceries", "amount": 25.00}]}
        )
        assert response.status_code == 200
        data = response.json()
    ...
    async def test_over_allocation_allowed(self, client: AsyncClient, seed_categories):
        ...
        response = await client.put(
            f"/api/v1/transactions/{txn_id}/splits",
            json={
                "splits": [{"tag": "bucket:groceries", "amount": 80.00}, {"tag": "bucket:shopping", "amount": 80.00}]
            },
        )
        assert response.status_code == 200
        data = response.json()
    ...
    async def test_replace_splits(self, client: AsyncClient, seed_categories):
        ...
        response = await client.put(
            f"/api/v1/transactions/{txn_id}/splits", json={"splits": [{"tag": "bucket:shopping", "amount": 75.00}]}
        )
        assert response.status_code == 200
        data = response.json()
    ...
    async def test_set_empty_splits(self, client: AsyncClient, seed_categories):
        ...
        response = await client.put(f"/api/v1/transactions/{txn_id}/splits", json={"splits": []})
        assert response.status_code == 200
    ...
    async def test_split_invalid_tag_format(self, client: AsyncClient, seed_categories):
        ...
        response = await client.put(
            f"/api/v1/transactions/{txn_id}/splits", json={"splits": [{"tag": "invalid-no-colon", "amount": 50.00}]}
        )
    ...
    async def test_split_non_bucket_tag_fails(self, client: AsyncClient, seed_categories):
        ...
        await client.post("/api/v1/tags/", json={"namespace": "occasion", "value": "birthday"})

        response = await client.put(
            f"/api/v1/transactions/{txn_id}/splits", json={"splits": [{"tag": "occasion:birthday", "amount": 50.00}]}
        )
        assert response.status_code == 400
    ...
    async def test_split_nonexistent_bucket_fails(self, client: AsyncClient, seed_categories):
        ...
        response = await client.put(
            f"/api/v1/transactions/{txn_id}/splits", json={"splits": [{"tag": "bucket:nonexistent", "amount": 50.00}]}
        )
        assert response.status_code == 400
    ...
    async def test_split_nonexistent_transaction(self, client: AsyncClient, seed_categories):
        ...
        response = await client.put("/api/v1/transactions/99999/splits", json={"splits": []})
        assert response.status_code == 404
    ...
    async def test_positive_transaction_splits(self, client: AsyncClient, seed_categories):
        ...
        response = await client.put(
            f"/api/v1/transactions/{txn_id}/splits", json={"splits": [{"tag": "bucket:savings", "amount": 200.00}]}
        )
        assert response.status_code == 200
        data = response.json()
