...
def detect_format(csv_content: str) -> ImportFormatType:
    """Auto-detect CSV format (BOFA vs AMEX)"""
    lines = csv_content.strip().split('\n')

    # Check first few lines for format indicators
    for line in lines[:10]:
        # AMEX has "Card Member" column
        if 'Card Member' in line and 'Account #' in line:
            return ImportFormatType.amex
        # BOFA has "Running Bal." column
        if 'Running Bal.' in line:
            return ImportFormatType.bofa
...
def extract_merchant_from_description(description: str, format_type: ImportFormatType) -> str:
    ...
    if format_type == ImportFormatType.bofa:
    ...
    elif format_type == ImportFormatType.amex:
...
def parse_bofa_csv(csv_content: str, account_source: str) -> List[Dict]:
    ...
    for row in reader:
        ...
        merchant = extract_merchant_from_description(description, ImportFormatType.bofa)
...
def parse_amex_csv(csv_content: str) -> List[Dict]:
    ...
    for row in reader:
        ...
        merchant = extract_merchant_from_description(description, ImportFormatType.amex)
