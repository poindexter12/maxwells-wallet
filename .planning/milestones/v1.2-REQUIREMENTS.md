# Requirements Archive: v1.2 Build System Modernization
...
- [x] **MISE-01**: mise manages all dev tooling (just, gum, node, python, uv) from `.mise.toml`
- [x] **MISE-02**: mise auto-installs correct tool versions on `cd` into project directory
- [x] **MISE-03**: Secrets loaded via mise `[env]` from gitignored `.env` file (replacing .envrc direct exports)
- [x] **MISE-04**: `.envrc` delegates to mise via `use mise` directive
...
- [x] **CI-01**: GitHub Actions use `mise-action` + `setup-just` to mirror local dev
- [x] **CI-02**: All CI workflow commands use `just <recipe>` instead of inline bash
- [x] **CI-03**: gum non-interactive fallback verified in all CI jobs

### Devcontainer (DEVC)

- [x] **DEVC-01**: Devcontainer uses mise feature for tool management
- [x] **DEVC-02**: `postCreateCommand` runs `mise trust && mise install && just setup`
- [x] **DEVC-03**: Tools available in new terminal sessions (mise activation in shell RC)

### Documentation (DOC)

- [x] **DOC-01**: CLAUDE.md updated — all `make` commands replaced with `just` equivalents
- [x] **DOC-02**: README.md updated — setup/dev instructions reference just
- [x] **DOC-03**: No remaining `make` command references in any docs (grep audit clean)
...
|-------------|-------|--------|
| MISE-01 | Phase 12 | Complete |
| MISE-02 | Phase 12 | Complete |
| MISE-03 | Phase 12 | Complete |
| MISE-04 | Phase 12 | Complete |
...
| DEVC-01 | Phase 14 | Complete |
| DEVC-02 | Phase 14 | Complete |
| DEVC-03 | Phase 14 | Complete |
