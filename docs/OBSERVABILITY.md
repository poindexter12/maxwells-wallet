...
Maxwell's Wallet includes production-ready observability features powered by OpenTelemetry and Prometheus.
...
All observability features are controlled via environment variables. Add these to your `.env` file:
...
OpenTelemetry tracing automatically instruments:
...
Access the health dashboard in the Admin UI under the **Health** tab:
...
- **Error rate** exceeding threshold (default: 5%)
- **P99 latency** exceeding threshold (default: 5000ms)
...
For development or testing, disable all observability:
