/**
 * Import page test IDs.
 */
export const IMPORT_IDS = {
  IMPORT_FILE_INPUT: 'import-file-input',
  IMPORT_ACCOUNT_SELECT: 'import-account-select',
  IMPORT_FORMAT_SELECT: 'import-format-select',
  IMPORT_PREVIEW_BUTTON: 'import-preview-button',
  IMPORT_CONFIRM_BUTTON: 'import-confirm-button',
  IMPORT_MODE_TOGGLE: 'import-mode-toggle',

  // Import Result
  IMPORT_RESULT: 'import-result',
  IMPORT_RESULT_TITLE: 'import-result-title',
  IMPORT_RESULT_IMPORTED: 'import-result-imported',
  IMPORT_RESULT_IMPORTED_COUNT: 'import-result-imported-count',
  IMPORT_RESULT_DUPLICATES: 'import-result-duplicates',
  IMPORT_RESULT_DUPLICATES_COUNT: 'import-result-duplicates-count',
  IMPORT_RESULT_FORMAT: 'import-result-format',
  IMPORT_RESULT_FORMAT_VALUE: 'import-result-format-value',
  IMPORT_RESULT_FILE_DETAILS: 'import-result-file-details',
  IMPORT_RESULT_CROSS_ACCOUNT_WARNING: 'import-result-cross-account-warning',
} as const;
